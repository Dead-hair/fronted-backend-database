...
def create_user():
    ...
    existing_user = User.query.filter_by(phone=phone).first()  # Ищем пользователя по номеру телефона
    if existing_user:
        return jsonify({'message': 'Пользователь с таким номером телефона уже существует!'}), 400  # Если есть, возвращаем ошибку
